...
def poll(kalshi_client: KalshiClient, session: Session) -> dict[str, Any]:
    ...
    if trade is not None and trade.ticker != ticker:
        ...
        if status in RESOLVED_STATUSES and result in ("yes", "no"):
            ...
            session.commit()
            return {"status": "closed_at_settlement", "ticker": trade.ticker, "result": trade.result, "pnl": trade.pnl}
        return {"status": "waiting_for_settlement", "ticker": trade.ticker}

    if trade is not None and trade.ticker == ticker:
        if yes_bid is None or trade.entry_price <= 0:
            return {"status": "monitoring", "ticker": ticker, "entry_price": trade.entry_price, "current_bid": yes_bid}

        gain_pct = (yes_bid - trade.entry_price) / trade.entry_price
        if gain_pct >= settings.btc_15min_profit_target_pct:
            series = SERIES_TICKER
            fee_multiplier = float(kalshi_client.get_series(series).get("series", {}).get("fee_multiplier", 1))
        ...
        return {"status": "monitoring", "ticker": ticker, "entry_price": trade.entry_price, "current_bid": yes_bid, "gain_pct": gain_pct}

    # No open position: enter this window if we haven't already traded it.
    if _already_traded(session, ticker):
        return {"status": "watching", "ticker": ticker, "yes_bid": yes_bid, "yes_ask": yes_ask}
    ...
    session.commit()
    return {"status": "opened", "ticker": ticker, "entry_price": yes_ask, "contracts": contracts, "fee": entry_fee}
